...
/// Fetch stored procedure definition and return an ALTER script suited for the DB.
/// - For MsSQL: returns an ALTER PROCEDURE script rebuilt from the existing definition
/// - For MySQL: returns the CREATE PROCEDURE definition with a comment explaining ALTER limitations
/// - Others: None
pub(crate) fn fetch_procedure_alter_script(
    connection: &models::structs::ConnectionConfig,
    database_name: Option<&str>,
    procedure_name: &str,
    // Note: procedure_name may be schema-qualified like [dbo].[proc] (MsSQL)
) -> Option<String> {
    let rt = tokio::runtime::Runtime::new().ok()?;
    ...
    rt.block_on(async {
        match connection_clone.connection_type {
            models::enums::DatabaseType::MySQL => {
                ...
                {
                    Ok(pool) => {
                        // SHOW CREATE PROCEDURE returns a row with column 'Create Procedure'
                        let qualified = format!("`{}`.`{}`", db_name.replace('`', "``"), proc_name.replace('`', "``"));
                        let query = format!("SHOW CREATE PROCEDURE {}", qualified);
                        match sqlx::query(&query).fetch_optional(&pool).await {
                            Ok(Some(row)) => {
                                use sqlx::Row;
                                // Column name varies by capitalization; try multiple
                                let def: Option<String> = row
                                ...
                                if let Some(create_stmt) = def {
                                    // MySQL cannot ALTER procedure body; keep CREATE with a helpful note
                                    let script = format!(
                                        "-- MySQL note: ALTER PROCEDURE cannot modify the body. Use DROP + CREATE instead.\n{}",
                                        create_stmt
                                    );
                                    Some(script)
                                } else {
            ...
            models::enums::DatabaseType::MsSQL => {
                ...
                let rt_res: Result<Option<String>, String> = async {
                    ...
                    // OBJECT_DEFINITION requires current DB context and object id
                    // Prefer OBJECT_DEFINITION(OBJECT_ID(N'[schema].[proc]'))
                    ...
                    while let Some(item) = stream.try_next().await.map_err(|e| e.to_string())? {
                        if let tiberius::QueryItem::Row(row) = item {
                            ...
                            if let Some(mut create_stmt) = def.map(|s| s.to_string()) {
                                // Transform CREATE to ALTER (first occurrence)
                                let upper = create_stmt.to_uppercase();
                                if let Some(pos) = upper.find("CREATE PROCEDURE") {
                                    create_stmt.replace_range(pos..pos + "CREATE".len(), "ALTER");
                                } else if let Some(pos) = upper.find("CREATE PROC") {
                                    create_stmt.replace_range(pos..pos + "CREATE".len(), "ALTER");
                                }
