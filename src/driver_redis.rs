...
pub(crate) fn load_redis_structure(tabular: &mut window_egui::Tabular, connection_id: i64, _connection: &models::structs::ConnectionConfig, node: &mut models::structs::TreeNode) {
...
pub(crate) fn fetch_tables_from_redis_connection(tabular: &mut window_egui::Tabular, connection_id: i64, database_name: &str, table_type: &str) -> Option<Vec<String>> {
              ...
              match table_type {
                            ...
                            "redis_keys" => {
                                   ...
                                   if let Ok(db_num) = database_name.trim_start_matches("db").parse::<i32>() {
                                          if (redis::cmd("SELECT").arg(db_num).query_async::<_, String>(&mut conn).await).is_ok() {
...
pub(crate) fn check_redis_database_has_keys(tabular: &mut window_egui::Tabular, connection_id: i64, database_name: &str) -> bool {
