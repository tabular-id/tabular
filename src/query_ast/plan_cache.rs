...
impl PlanCache {
    ...
    pub fn get(&self, key: &str) -> Option<PlanEntry> {
        ...
        if out.is_some() { self.hits.fetch_add(1, std::sync::atomic::Ordering::Relaxed); } else { self.misses.fetch_add(1, std::sync::atomic::Ordering::Relaxed); }
        out
    }
    pub fn insert(&self, key: String, entry: PlanEntry) {
        if let Ok(mut guard) = self.inner.lock() {
            ...
            let versioned = format!("v{}::{}", CACHE_VERSION, key);
            if !map.contains_key(&versioned) { order.push_back(versioned.clone()); }
            map.insert(versioned.clone(), entry);
            while order.len() > MAX_ENTRIES {
                if let Some(old_key) = order.pop_front() { map.remove(&old_key); }
            }
        }
    }
    pub fn stats(&self) -> (u64,u64) { (self.hits.load(std::sync::atomic::Ordering::Relaxed), self.misses.load(std::sync::atomic::Ordering::Relaxed)) }
}
