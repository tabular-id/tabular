...
impl DatabaseExecutor for SqliteExecutor {
    ...
    async fn execute_query(
        ...
        debug!("SqliteExecutor: executing query on connection {}", connection_id);
        debug!("SQL: {}", sql);
            ...
            .map(|row| {
                ...
                for i in 0..row.columns().len() {
                    let value = if let Ok(v) = row.try_get::<Option<String>, _>(i) {
                        ...
                        v.map(|b| if b { "1" } else { "0" }.to_string()).unwrap_or_default()
                    } else {
                        String::new()
                    };
                    row_data.push(value);
        ...
        debug!("SqliteExecutor: query returned {} rows, {} columns", data.len(), headers.len());
