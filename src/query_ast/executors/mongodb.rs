...
impl MongoDbExecutor {
    ...
    fn parse_simple_select(sql: &str) -> Result<(String, Option<Document>, Option<i64>), QueryAstError> {
...
impl DatabaseExecutor for MongoDbExecutor {
    ...
    async fn execute_query(
        ...
        debug!("MongoDbExecutor: executing query on connection {}", connection_id);
        debug!("SQL: {}", sql);
        ...
        let db_name = database_name.ok_or_else(|| QueryAstError::Semantic(
            "MongoDB requires database name".to_string()
        ))?;
        ...
        let data: Vec<Vec<String>> = docs.iter()
            .map(|doc| {
                headers.iter()
                    .map(|header| {
                        doc.get(header)
...
mod tests {
    ...
    fn test_query_validation() {
        ...
        assert!(executor.validate_query("UPDATE users SET name = 'x'").is_err());
        assert!(executor.validate_query("SELECT * FROM users JOIN orders").is_err());
    }
