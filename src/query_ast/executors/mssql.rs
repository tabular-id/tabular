...
impl MssqlExecutor {
    ...
    fn get_config(connection_id: i64) -> Result<Arc<crate::driver_mssql::MssqlConfigWrapper>, QueryAstError> {
...
impl DatabaseExecutor for MssqlExecutor {
    ...
    async fn execute_query(
        ...
        debug!("MssqlExecutor: executing query on connection {}", connection_id);
        debug!("SQL: {}", sql);
        ...
        // Execute the query
        let mut stream = client.query(&final_sql, &[])
            .await
            .map_err(|e| QueryAstError::Execution {
        ...
        // Process result sets
        while let Some(item) = stream.try_next().await.map_err(|e| QueryAstError::Execution {
            query: final_sql.clone(),
            reason: e.to_string(),
        })? {
            match item {
        ...
        debug!("MssqlExecutor: query returned {} rows, {} columns", all_rows.len(), headers.len());
