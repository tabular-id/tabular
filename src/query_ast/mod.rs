...
pub fn compile_single_select(
    ...
    use rewrite::{Pagination, apply_basic_rewrites};
    ...
    fn hash_expr(e: &logical::Expr, h: &mut impl Hasher) {
        ...
        match e {
            E::Column(c) | E::StringLiteral(c) | E::Number(c) | E::Raw(c) => {
                c.to_ascii_lowercase().hash(h)
            }
            E::BinaryOp { left, op, right } => {
                op.to_ascii_lowercase().hash(h);
                hash_expr(left, h);
                hash_expr(right, h);
            }
            ...
            E::Alias { expr, alias } => {
                alias.to_ascii_lowercase().hash(h);
                hash_expr(expr, h);
            }
            E::Null => {}
            ...
            E::Subquery { sql, correlated } => {
                sql.trim().to_ascii_lowercase().hash(h);
                correlated.hash(h);
            }
            E::WindowFunc {
                name,
                ...
                for (o, asc) in order_by {
                    hash_expr(o, h);
                    asc.hash(h);
                }
                if let Some(f) = frame {
                    f.to_ascii_lowercase().hash(h);
                }
            }
            E::Star => {
                "*".hash(h);
    ...
    fn hash_plan(p: &LogicalQueryPlan, h: &mut impl Hasher) {
        ...
        match p {
            ...
            L::With { ctes, input } => {
                for (n, s) in ctes {
                    n.to_ascii_lowercase().hash(h);
                    s.to_ascii_lowercase().hash(h);
                }
                hash_plan(input, h);
            }
            L::SetOp { left, right, op } => {
                (*op as u8).hash(h);
                hash_plan(left, h);
                hash_plan(right, h);
            }
            L::TableScan { table, alias } => {
                table.to_ascii_lowercase().hash(h);
                if let Some(a) = alias {
                    a.to_ascii_lowercase().hash(h);
                }
            }
            L::SubqueryScan {
                sql,
    ...
    let pre_key = format!(
        "pre{}::{:?}::{:?}::{}",
        fp_struct, db_type, pagination, inject_auto_limit
    );
    if let Some(entry) = PlanCache::global().get(&pre_key) {
    ...
    let cache_key = format!(
        "plan{}::{:?}::{:?}::{}",
        logical_fp, db_type, pagination, inject_auto_limit
    );
    if let Some(entry) = PlanCache::global().get(&cache_key) {
        return Ok((entry.sql, entry.headers));
    }
    let pagination = pagination.map(|(page, size)| Pagination {
        page,
        page_size: size,
    });
    ...
    if let logical::LogicalQueryPlan::With { ctes, .. } = &plan
        && !ctes.is_empty()
    {
        remaining_ctes = Some(ctes.iter().map(|(n, _)| n.clone()).collect());
    }
...
#[cfg(feature = "query_ast")]
pub fn take_last_debug() -> Option<(u64, String, Option<Vec<String>>)> {
    let mut out = None;
    STORE_DEBUG.with(|s| {
        out = s.borrow_mut().take();
    });
    out
}
...
#[cfg(feature = "query_ast")]
pub fn last_rewrite_rules() -> Vec<String> {
    crate::query_ast::rewrite::take_last_rules()
}

#[cfg(feature = "query_ast")]
pub fn plan_structural_hash(
    raw: &str,
    db_type: &DatabaseType,
    pagination: Option<(u64, u64)>,
    inject_auto_limit: bool,
) -> Result<(u64, String), QueryAstError> {
    use parser::parse_single_select_to_plan;
    use std::collections::hash_map::DefaultHasher;
    use std::hash::{Hash, Hasher};
    ...
    fn hash_plan(p: &LogicalQueryPlan, h: &mut impl Hasher) {
        ...
        match p {
            ...
            | L::Limit { input, .. }
            | L::Having { input, .. }
            | L::With { input, .. } => hash_plan(input, h),
            L::Join {
                left,
                right,
                on,
    ...
    hash_plan(&plan, &mut hasher);
    let structural = hasher.finish();
    let cache_key = format!(
        "{:x}::{:?}::{:?}::{}",
        structural, db_type, pagination, inject_auto_limit
    );
...
pub fn debug_plan(raw: &str, db_type: &DatabaseType) -> Result<String, QueryAstError> {
    ...
    fn fmt(plan: &LogicalQueryPlan, indent: usize, out: &mut String) {
        ...
        match plan {
            L::TableScan { table, alias } => {
                out.push_str(&format!("{}TableScan({} alias={:?})\n", pad, table, alias));
            }
            L::SubqueryScan { alias, .. } => {
                out.push_str(&format!("{}SubqueryScan(alias={})\n", pad, alias));
            }
            L::Projection { exprs, input } => {
            ...
            L::Join { left, right, .. } => {
                ...
                fmt(left, indent + 1, out);
                fmt(right, indent + 1, out);
            }
            L::Having { predicate, input } => {
            ...
            L::With { ctes, input } => {
                out.push_str(&format!(
                    "{}With ctes={:?}\n",
                    pad,
                    ctes.iter().map(|(n, _)| n).collect::<Vec<_>>()
                ));
                fmt(input, indent + 1, out);
            }
            L::SetOp { left, right, op } => {
                out.push_str(&format!("{}SetOp {:?}\n", pad, op));
                fmt(left, indent + 1, out);
                fmt(right, indent + 1, out);
            }
        }
...
#[cfg(feature = "query_ast")]
pub fn plan_metrics(raw: &str) -> Result<(usize, usize, usize, usize, usize), QueryAstError> {
    // (nodes, depth, subqueries_total, subqueries_correlated, windows)
    use parser::parse_single_select_to_plan;
    let plan = parse_single_select_to_plan(raw)?;
    fn walk(p: &LogicalQueryPlan, depth: usize, stats: &mut (usize, usize, usize, usize, usize)) {
        stats.0 += 1;
        stats.1 = stats.1.max(depth);
        use logical::LogicalQueryPlan as L;
        match p {
            L::Projection { input, .. }
            | L::Filter { input, .. }
            ...
            | L::Group { input, .. }
            | L::Having { input, .. }
            | L::With { input, .. } => walk(input, depth + 1, stats),
            L::Join { left, right, .. } => {
                walk(left, depth + 1, stats);
                walk(right, depth + 1, stats);
            }
            L::SetOp { left, right, .. } => {
                walk(left, depth + 1, stats);
                walk(right, depth + 1, stats);
    ...
    fn count_expr(e: &logical::Expr, subs: &mut usize, correlated: &mut usize, wins: &mut usize) {
        use logical::Expr as E;
        match e {
            E::Subquery { correlated: c, .. } => {
                *subs += 1;
                if *c {
                    *correlated += 1;
                }
            ...
            E::WindowFunc { .. } => *wins += 1,
            E::Alias { expr, .. } => count_expr(expr, subs, correlated, wins),
            E::BinaryOp { left, right, .. } => {
                count_expr(left, subs, correlated, wins);
                count_expr(right, subs, correlated, wins);
            }
            E::FuncCall { args, .. } => {
                for a in args {
                    count_expr(a, subs, correlated, wins);
                }
            }
            E::Case {
                when_then,
                ...
                if let Some(o) = operand {
                    count_expr(o, subs, correlated, wins);
                }
                for (w, t) in when_then {
                    count_expr(w, subs, correlated, wins);
                    count_expr(t, subs, correlated, wins);
                }
                if let Some(e2) = else_expr {
                    count_expr(e2, subs, correlated, wins);
                }
            }
            E::InList { expr, list, .. } => {
                count_expr(expr, subs, correlated, wins);
                for l in list {
                    count_expr(l, subs, correlated, wins);
                }
            }
            E::Like { expr, pattern, .. } => {
                count_expr(expr, subs, correlated, wins);
                count_expr(pattern, subs, correlated, wins);
            }
            E::Not(inner) => count_expr(inner, subs, correlated, wins),
            E::IsNull { expr, .. } => count_expr(expr, subs, correlated, wins),
            _ => {}
        }
    }
    // Drill down to find projection expressions for counting subqueries/windows
    fn collect_projection(p: &LogicalQueryPlan, out: &mut Vec<logical::Expr>) {
        use logical::LogicalQueryPlan as L;
        match p {
            L::Projection { exprs, input } => {
                out.extend(exprs.clone());
                collect_projection(input, out);
            }
            L::Filter { input, .. }
            | L::Sort { input, .. }
            | L::Limit { input, .. }
            ...
            | L::Group { input, .. }
            | L::Having { input, .. }
            | L::With { input, .. } => collect_projection(input, out),
            L::Join { left, right, .. } | L::SetOp { left, right, .. } => {
                collect_projection(left, out);
                collect_projection(right, out);
            }
            L::TableScan { .. } | L::SubqueryScan { .. } => {}
        }
    }
    let mut stats = (0, 0, 0, 0, 0);
    walk(&plan, 0, &mut stats);
    let mut exprs = Vec::new();
    collect_projection(&plan, &mut exprs);
    for e in &exprs {
        count_expr(e, &mut stats.2, &mut stats.3, &mut stats.4);
    }
    Ok(stats)
}
...
fn infer_headers_from_plan(plan: &LogicalQueryPlan) -> Vec<String> {
    use logical::Expr as E;
    use logical::LogicalQueryPlan as L;
    fn find_projection(p: &L) -> Option<&Vec<E>> {
        match p {
