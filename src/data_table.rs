...
use crate::{connection, driver_mssql, export, models, spreadsheet::SpreadsheetOperations, window_egui
};
        ...
        if !tabular.current_table_headers.is_empty() || !tabular.current_table_name.is_empty() {
            ...
            let mut target_cell_rect: Option<egui::Rect> = None;
                        let grid_response = egui::Grid::new("table_data_grid")
                            ...
                            .show(ui, |ui| {
                                ...
                                for (col_index, header) in headers.iter().enumerate() {
                                    ...
                                    let column_width = if Some(col_index) == error_column_index {
                                        ...
                                        if get_column_width(tabular, col_index) <= 180.0 { // Default width
                                ...
                                for (row_index, row) in current_table_data.iter().enumerate() {
                                    let is_selected_row = selected_rows.contains(&row_index) || selected_row == Some(row_index);
                                    ...
                                    for (col_index, cell) in row.iter().enumerate() {
                                        let is_selected_cell = tabular.selected_cell == Some((row_index, col_index));
                                        let is_selected_col = tabular.selected_columns.contains(&col_index);
                                        let column_width = if Some(col_index) == error_column_index {
                                            get_column_width(tabular, col_index).max(100.0)
                                        } else {
                                        ...
                                        ui.allocate_ui_with_layout(
                                            ...
                                            |ui| {
                                                ...
                                                if tabular.is_table_browse_mode && cell_response.double_clicked() {
                                                ...
                                                let is_editing_this_cell = tabular.spreadsheet_state.editing_cell == Some((row_index, col_index));
                                                ...
                                                if is_editing_this_cell {
                                                    ...
                                                    let mut edit_text = tabular.spreadsheet_state.cell_edit_text.clone();
                            ...
                            ui.separator();
                                                        if tabular.is_table_browse_mode && ui.button("📋 Add New Row").clicked() {
                                                        ...
                                                        if !tabular.selected_rows.is_empty() && ui.button("📄 Copy Selected Rows as CSV").clicked() {
                                                            if let Some(csv) = copy_selected_rows_as_csv(tabular) { ui.ctx().copy_text(csv); }
                                                            ui.close();
                                                        }
                                                        if !tabular.selected_columns.is_empty() && ui.button("📄 Copy Selected Columns as CSV").clicked() {
                                                            if let Some(csv) = copy_selected_columns_as_csv(tabular) { ui.ctx().copy_text(csv); }
                                                            ui.close();
                                                        }
                                                        if let Some(selected_row_idx) = tabular.selected_row
                                                            && ui.button("📄 Copy Row as CSV").clicked() {
                                                                if let Some(row_data) = tabular.current_table_data.get(selected_row_idx) {
                                                        ...
                                                        if ui.button("📄 Export to CSV").clicked() {
                                                            export::export_to_csv(&tabular.all_table_data, &tabular.current_table_headers, &tabular.current_table_name);
                                                            ui.close();
                                                        }
                                                        if ui.button("📊 Export to XLSX").clicked() {
                                                            export::export_to_xlsx(&tabular.all_table_data, &tabular.current_table_headers, &tabular.current_table_name);
                                                            ui.close();
                                                        }
                                                        ui.separator();
                                                        if tabular.is_table_browse_mode
            ...
            grid_response.response.context_menu(|ui| {
                ...
                if refresh_request_data { refresh_current_table_data(tabular); }
...
pub(crate) fn sort_table_data(tabular: &mut window_egui::Tabular, column_index: usize, ascending: bool) {
...
pub(crate) fn apply_sql_filter(tabular: &mut window_egui::Tabular) {
    ...
    // Determine current target
        ...
        if !tabular.request_structure_refresh && tabular
            ...
            .and_then(|t| t.connection_id)
        {
            ...
            if let Some(ct) = db_type {
                let query = match ct {
                    ...
                    models::enums::DatabaseType::MsSQL => driver_mssql::build_mssql_select_query(db_name.clone(), table.clone()),
                    _ => String::new(),
                };
                if !query.is_empty()
                    && let Some((headers, data)) = connection::execute_query_with_connection(tabular, conn_id, query) {
                        tabular.current_table_headers = headers;
                        tabular.current_table_data = data.clone();
                        tabular.all_table_data = data;
                        tabular.total_rows = tabular.all_table_data.len();
                        ...
                        let snapshot: Vec<Vec<String>> = tabular.all_table_data.iter().take(100).cloned().collect();
                        let headers_clone = tabular.current_table_headers.clone();
                        crate::cache_data::save_table_rows_to_cache(tabular, conn_id, &db_name, &table, &headers_clone, &snapshot);
                            ...
                            .await
                        {
                            Ok(rows) => {
                                ...
                                for r in rows {
                                    ...
                                    let unique_flag: Option<i64> =
                                        r.try_get("unique").ok().flatten();
                                    if let Some(nm) = name_opt {
                                        ...
                                        if let Ok(crows) = sqlx::query(&info_q)
                                            .fetch_all(sqlite_pool.as_ref())
                                            .await
                                        {
                                            for cr in crows {
...
pub(crate) fn render_structure_columns_editor(tabular: &mut window_egui::Tabular, ui: &mut egui::Ui) {
        let headers = [
            "#",
            "column_name",
            "data_type",
...
pub(crate) fn render_drop_column_confirmation(tabular: &mut window_egui::Tabular, ctx: &egui::Context) {
...
fn commit_new_index(tabular: &mut window_egui::Tabular) {
        ...
        let stmt = match conn.connection_type {
            models::enums::DatabaseType::MySQL => {
                ...
                let unique = if tabular.new_index_unique { "UNIQUE " } else { "" };
                format!(
...
pub(crate) fn render_drop_index_confirmation(tabular: &mut window_egui::Tabular, ctx: &egui::Context) {
...
pub(crate) fn handle_row_click(tabular: &mut window_egui::Tabular, row_index: usize, modifiers: egui::Modifiers) {
...
pub(crate) fn handle_column_click(tabular: &mut window_egui::Tabular, col_index: usize, modifiers: egui::Modifiers) {
        if modifiers.command {
            // Cmd/Ctrl toggles
...
pub(crate)fn set_column_width(tabular: &mut window_egui::Tabular, column_index: usize, width: f32) {
...
pub(crate)fn update_current_page_data(tabular: &mut window_egui::Tabular) {
        ...
        let end_index = ((tabular.current_page + 1) * tabular.page_size).min(tabular.all_table_data.len());
